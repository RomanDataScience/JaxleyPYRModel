...
params["passive"] = {
    "somatic_capacitance": 1.0,
    "apical_capacitance": 2.0,
    "axonal_capacitance": 1.0,
    "basal_capacitance": 1.0,
    "whole_Leak_eLeak": -75.0,
}
bounds = {
    "apical_H_scale": [0.1, 5.0],
    "basal_H_scale": [0.1, 5.0],
    ...
    "RaTuft": [20.0, 100.0],
    "DistHalfRm": [20.0, 400.0],
    "DistHalfRa": [20.0, 300.0],
    "SlopeRm": [1.0, 50.0],
    "SlopeRa": [1.0, 50.0],
    "somatic_capacitance": [0.5, 3.0],
    "apical_capacitance": [0.5, 4.0],
    "axonal_capacitance": [0.5, 3.0],
    "basal_capacitance": [0.5, 3.0],
    "whole_Leak_eLeak": [-90.0, -55.0],
}
...
FIT_PARAMETER_ALIASES = {
    "somatic_capacitance": "capacitance",
    "apical_capacitance": "capacitance",
    "axonal_capacitance": "capacitance",
    "basal_capacitance": "capacitance",
    "whole_Leak_eLeak": "Leak_eLeak",
}


def fitted_parameter_key(name):
    return FIT_PARAMETER_ALIASES.get(name, name)


def fitted_parameter_group(cell, name):
    if name.startswith("apical_"):
        return cell.apical
    if name.startswith("somatic_"):
        return cell.soma
    if name.startswith("axonal_"):
        return cell.axon
    if name.startswith("basal_"):
        return cell.basal
    if name.startswith("whole_"):
        return cell
    raise ValueError(f"Unknown direct fit parameter group for {name}")
...
def passive_profile_values(distances, passive_params=None):
    profile = dict(params["passive_profile"])
    if passive_params is not None:
        profile.update(passive_params)

    xdist = jnp.minimum(jnp.asarray(distances), PASSIVE_PROFILE_DISTANCE_CAP)
    rm = profile["RmTuft"] + (profile["RmSoma"] - profile["RmTuft"]) / (
        1.0 + _my_exp((xdist - profile["DistHalfRm"]) / profile["SlopeRm"])
    )
    ra = profile["RaTuft"] + (profile["RaSoma"] - profile["RaTuft"]) / (
        1.0 + _my_exp((xdist - profile["DistHalfRa"]) / profile["SlopeRa"])
    )
    return 1.0 / rm, ra
...
def apical_h_conductance(cell, scale=1.0):
    ...
    baseline = (
        -0.8696 + 2.087 * jnp.exp(distances * 0.0031)
    ) * BASE_APICAL_GH
...
def set_h_conductance(cell, apical_scale=1.0, basal_scale=1.0, state=None):
    ...
    state = cell.basal.data_set(
        "basal_H_gH",
        BASE_BASAL_GH * basal_scale,
        state,
    )
    return state


def set_passive_profile(cell, passive_params=None):
    leak_g, axial_resistivity = passive_profile_values(
        cell.nodes["dist_from_soma"].to_numpy(dtype=float),
        passive_params,
    )
    cell.set("Leak_gLeak", np.asarray(leak_g))
    cell.set("axial_resistivity", np.asarray(axial_resistivity))
    return cell
...
def set_fitted_parameters(cell, keys, values, state=None):
    ...
    if "apical_H_scale" in values_by_key:
        state = cell.apical.data_set(
            "apical_H_gH",
            apical_h_conductance(cell, values_by_key["apical_H_scale"]),
            state,
        )

    if "basal_H_scale" in values_by_key:
        state = cell.basal.data_set(
            "basal_H_gH",
            BASE_BASAL_GH * values_by_key["basal_H_scale"],
            state,
        )

    special_keys = set(PASSIVE_PROFILE_KEYS) | set(H_SCALE_KEYS)
    for key in keys:
        if key in special_keys:
            continue
        state = fitted_parameter_group(cell, key).data_set(
            fitted_parameter_key(key),
            values_by_key[key],
            state,
        )
    return state
...
def L5PC(d_lambda=0.1):
    ...
    cell = update_number_compartments(cell, d_lambda=d_lambda)
    cell.initialize()
    ########## APICAL ##########
    ...
    cell.apical.set(
        "apical_H_gH",
        np.asarray(apical_h_conductance(cell, scale=1.0)),
    )
    ...
    cell.basal.set("basal_H_gH", BASE_BASAL_GH)
