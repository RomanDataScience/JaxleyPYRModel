...
from model import L5PC, SEGMENTED_TRACES_DIR, bounds, params
...
def parameter_group(cell, key):
    if key.startswith("apical_"):
        return cell.apical
    if key.startswith("somatic_"):
        return cell.soma
    if key.startswith("axonal_"):
        return cell.axon
    raise ValueError(f"Unknown parameter group for {key}")


def parameter_state(cell, keys, values):
    state = None
    for i, key in enumerate(keys):
        state = parameter_group(cell, key).data_set(key, values[i], state)
    return state
...
def main():
    ...
    keys = parameter_keys()
    lower = jnp.asarray([bounds[key][0] for key in keys], dtype=jnp.float64)
    upper = jnp.asarray([bounds[key][1] for key in keys], dtype=jnp.float64)
    ...
    def simulate(fit_params):
        pstate = parameter_state(cell, keys, fit_params)
